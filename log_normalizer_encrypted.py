...
def process_zip(zip_path, output_csv_path, buffer_size=1000, password=None):
    start_time = time.time()
    ...
    with zipfile.ZipFile(zip_path, 'r') as zf, open(output_csv_path, 'w', newline='', encoding='utf-8') as out_csv:
        ...
        # If password is provided, convert it to bytes
        ...
        for name in zf.namelist():
            ...
            print(f"📄 Processing file: {name}")
            try:
                with zf.open(name, 'r', pwd=pw_bytes) as f:
                    wrapper = TextIOWrapper(f, encoding='utf-8', errors='ignore')
                    for line in wrapper:
                        row = extract_info(line)
                        if row:
                ...
                print(f"❌ Skipping {name} (possibly incorrect password or unsupported encryption): {e}")
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--password", help="Password for encrypted ZIP file", default=None)
    args = parser.parse_args()

    process_zip(args.zip_path, args.output_csv, args.buffer, args.password)